...
Restock is the one dead moment in the game. You drive back to the shop, hold near the dock for a flat
1.0 second, and three bags appear (`src/80_game.js`, the `/* restock at the shop */` block, ~line 570;
the LOADING bar is `drawRestock`, ~line 747). I want that to become a kitchen mini-game.

I already know what mini-game. I built it before, in a previous game, and it was the best thing in
that project. I want it **rebuilt here, in this game's idiom**. Your job is the translation, not the
invention.

## Part 1 — The Build (the mechanic, as it worked)

An order arrives. It has 1–8 **items** (a customer orders more than one thing). Each item is a menu
item — hard taco, bean burrito, sancho, nachos — and each menu item has a **fixed ingredient
sequence**. You assemble it by clicking ingredients from a grid **in the correct order**, then hit a
finishing action to close the item; when every item on the order is finished, you SERVE the order.

The rules that made it work, in priority order:

1. **The recipes are hidden.** This is the entire skill and the thing I care most about. An early
   version showed the ingredient list with the next one highlighted, and it destroyed the game — there
   was no cooking skill left, just following arrows. The player sees **how many steps remain** (a row
   of pips) and nothing else. You have to *know* that a sancho is tortilla → beef → CCQ → onion →
   wrap. Learning the menu **is** the mastery curve.
2. **Strict order, no partial credit.** Right ingredient in the wrong position is a mistake.
3. **A wrong click flashes red and is counted.** It doesn't undo your progress — it costs you accuracy
   and time, and it feeds the bark (below).
4. **One order on screen at a time**, header reading `ORDER #N — n of M`. Items build strictly in
   sequence and **auto-advance**: finish the current one, it slides off, the next slides in. No manual
   item picking. (I tried three tickets side by side; it was visual soup. One at a time, conveyor
   style, is correct.)
5. **Ingredient grid of 10–12**, so the player has to *discriminate*. When every ingredient is used by
   every recipe the grid is just a sequence to walk, not a choice.
6. **Everything is abbreviated, old-POS style** — `HRDTC`, `BFTSLD`, `CCQ`, `LET`, `CHZ`. It saves
   space, it reads period-authentic, and it's one more thing the player has to internalise to become a
   line cook. This never shipped in the old project and I still want it.
7. **A patience timer per order, ending in a walkout.** The walkout was a *beat*, not a flash: the
   work area dimmed, the interface buttons hid, and a customer portrait came up with an angry quote.
8. **Escalating correction barks.** The kitchen veteran watching you had three tiers keyed to your
   mistake count — 1 mistake: *"Easy, kid — slow down."* 3: *"Focus up. I'm watching."* 6: *"Last
   chance, kid."* Cheap to build, and it did more for the pressure than any timer.
9. **Scored 1–5 stars**: `served − walkouts − floor(mistakes / 3)`, clamped.
10. **Sound is load-bearing.** Ingredient clicks, order arrival, the walkout, and the SERVE moment.
    The feel I wrote down at the time was **"slot-machine fun"** — that's the target. A muted version
    of this mini-game is a boring version.

Difficulty scaled by lengthening recipes, widening the menu pool, adding items per order, and
tightening patience.

## Part 2 — The feel target: this game, not that one

**Carnage Asada's look and feel are right and are not up for revision.** The frantic pace, the arcade
read, the CRT pass, the palette discipline, the procedural chunky sprites, the chip audio — that's the
product. The Build is being brought *into* this game's aesthetic, not bolted onto the side of it. If
any detail of Part 1 fights the way this game looks or sounds, **this game wins** — say so and adapt it.
...
- **Tempo — the big one.** The Build ran at 35–60 *second* patience windows across a 1–4 minute round.
  Carnage Asada's whole shift is 110 seconds and a round trip is ~9. The mechanic has to be compressed
  by roughly an order of magnitude to belong here. A 40-second order card in this game is absurd. Work
