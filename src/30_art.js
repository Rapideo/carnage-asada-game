...
const Art = {
  ...
  /* ---------- sprayed wordmark ----------------------------
     Two passes: a mist halo, then the solid core last so the letterforms
     stay crisp. Baked once at boot from a seeded rng, so it is deterministic
  ...
  mkSprayText(str, s, col, seed) {
    ...
    // TAIL only has to clear the mist now that there are no drips
    const PAD = 10, TAIL = 8;
    ...
    const RAD = s * 1.25;   // counters are only 1-3 cells on the 7x9 grid
    ...
    // 2. solid core on top, lightly mottled so it is not a flat fill
    for (const [a, b] of cells) {
  ...
  mkBadge() {
    ...
    // At 92px the words just overspill the 88px face — the real sticker
    // breaks its circle the same way.
    keyline(x, 'TACO', c, 19, 4, 2, PAL.gold);
    keyline(x, 'SHOP', c, 51, 4, 2, PAL.gold);
