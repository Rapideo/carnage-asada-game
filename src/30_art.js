...
/* Graffiti alphabet for the title wordmark — 9x11, hand-authored, and only
   the characters "CARNAGE ASADA" needs. The 5x7 game font has no room for
   letterform character: every stroke is 1 cell, so sprayed bitmap text reads
   as the game font wearing drips. These have weight, angled terminals and
   tapering strokes. Title art only — the game font is untouched. */
const GRAF_W = 9, GRAF_H = 11;
const GRAF_SRC = {
  'A': '...###...,...###...,..##.##..,..##.##..,.##...##.,.##...##.,' +
       '.#######.,.#######.,##.....##,##.....##,##.....##',
  'C': '..#####..,.#######.,###...###,###......,###......,###......,' +
...
const GRAF = Object.create(null);
for (const ch in GRAF_SRC) {
  const rows = GRAF_SRC[ch].split(','), bits = [];
  for (let y = 0; y < GRAF_H; y++) {
    const row = rows[y] || '';
    for (let gx = 0; gx < GRAF_W; gx++) if (row[gx] === '#') bits.push(gx, y);
  }
  GRAF[ch] = bits;
}
/** width of a graffiti string in px at a given scale (1 cell letter gap) */
function grafW(str, s) { return str.length ? (str.length * (GRAF_W + 1) - 1) * s : 0; }
...
const Art = {
  ...
  buildMisc(rng) {
    ...
    // scale 2 on a 9x11 grid ≈ the same cap height as scale 3 on the 5x7 font
    this.wordmark = this.mkSprayText('CARNAGE ASADA', 2, PAL.red, 77311);
  },
  ...
  mkSprayText(str, s, col, seed) {
    ...
    // lit cells in unscaled glyph space, from the graffiti alphabet
    const cells = [];
    let gx = 0;
    ...
    const t = mkCanvas(grafW(str, s) + PAD * 2, GRAF_H * s + PAD + TAIL);
    const x = t.x;
    ...
    for (const [a, b] of cells) {
      const n = 4 + r.int(4);   // 9x11 glyphs have ~3x the cells of the 5x7 font
      for (let i = 0; i < n; i++) {
    ...
    const wdt = s;
    for (const [a, b] of cells) {
      // 9 columns per letter instead of 5, so the per-column odds drop to
      // keep roughly the same number of runs across the word
      if (b !== lowest.get(a) || !r.chance(0.20)) continue;
  ...
  mkBadge() {
    ...
    // Ring widths stay ~3px rather than shrinking with the disc — thinner
    // than that and they start dropping out at 1x.
    disc(x, c, c, 44, '#ffffff');
    disc(x, c, c, 41, PAL.ink);
    disc(x, c, c, 38, '#ffffff');
    disc(x, c, c, 36, PAL.ink);
    disc(x, c, c, 34, PAL.jade);
    ...
    x.globalAlpha = 0.09;
    disc(x, c - 13, c - 14, 16, '#ffffff');
    x.globalAlpha = 1;

    // Wordmark. NOT textOut(): that offsets its outline by the glyph scale,
    // so the eight copies merge into a solid black slab past about 2x. The
    // logo wants a constant-width keyline, independent of letter size.
    // At 69px the words just overspill the 68px face onto the rings — the
    // real sticker does that too.
    keyline(x, 'TACO', c, 16, 3, 2, PAL.gold);
    keyline(x, 'SHOP', c, 43, 3, 2, PAL.gold);
    keyline(x, 'EST. 1970', c, 70, 1, 1, PAL.gold);
    return t.c;
  },
